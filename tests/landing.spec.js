...
test("admin dashboard is wired to protected lead commands", () => {
  ...
  assert.ok(adminScript.includes("dentalMotionResendSent"));
  ...
  assert.ok(adminScript.includes("/api/admin/leads/test-email"));
  assert.ok(adminScript.includes("dentalMotionAdminToken"));
  assert.ok(server.includes('decodedPath === "/admin"'));
  assert.ok(server.includes('path === "/api/admin/leads/test-email"'));
  assert.ok(server.includes('".mp4": "video/mp4"'));
});
